...
const IngressInfo = ({
  ...
  const ingressInfo = [
    {
      title: 'Domain Name',
      paths: ['spec.rules.0.host', 'spec.tls.0.hosts.0'],
    },
    { title: 'Path', paths: ['spec.rules.0.http.paths.0.path'] },
  ];
  ...
  const { loading, error } = useAsync(async (): Promise<IngressRes> => {
    ...
    const response = await fetch(`${baseUrl}/get_ingress`, {
      method: 'POST',
      headers: {
    ...
    ingressInfo.forEach((info, i) => {
      const val = resolveJsonPath(data?.ingress_json as Ingress, info.paths[0]);
  ...
  const saveHandler = async () => {
    ...
    const baseUrl = await discoveryApi.getBaseUrl('ubility');
    const response = await fetch(`${baseUrl}/edit_apply_ingress`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
