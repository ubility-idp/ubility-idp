...
# def ____execute_shell_command(command, type=None):
#     print(command)
...
#         print(err1)
#         return {'error': err1}
#     return {"message": result.communicate()[0].decode("utf-8")}


def execute_shell_command(command, type=None):
    result = subprocess.Popen(
        command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def aws_eks_update_kube_config(body):
    ...
    if not ("aws_access_key_id" in body and "aws_secret_access_key" in body  and "region" in body and "cluster_name" in body):
        return {"message": "error missing input key(s)"}
        
    if not (body["aws_access_key_id"] and body["aws_secret_access_key"] and body["region"] and body["cluster_name"]):
...
def azure_aks_update_kube_config(body):
    ...
    if not ("client_id" in body and "client_secret" in body and "tenant" in body and "subscription_id" in body  and "resourcegroup_name" in body and "cluster_name" in body):
        return {"message": "error missing input key(s)"}
        
    if not (body["client_id"] and body["client_secret"] and body["tenant"] and body["subscription_id"] and body["resourcegroup_name"] and body["cluster_name"]):
        return {"message": "error missing input data"}
...
def execute_commads_list(commands_list):
    ...
    cmdWithJsonOutput = ["kubectl apply", "kubectl run", "kubectl autoscale",
                                    "kubectl scale", "kubectl get", "kubectl edit", "kubectl create"]
        ...
        if len(word_list) >= 2:
            ...
            if wordsToCompare in cmdWithJsonOutput:
                if cmd.__contains__("-o") and cmd.__contains__(" json"):
                    ## exist
                    result_list.append(
                        execute_commads_with_json_output(cmd, "bash"))
                else:
                    ## add
                    result_list.append(
